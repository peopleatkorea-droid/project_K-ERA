    ...
    aggregations: [
      {
        ...
        dp_accounting: {
          ...
          assumptions: ["client_delta_noise", "gaussian_rdp", "poisson_subsampling", "no_secure_aggregation"],
          accounted_updates: 2,
          accounted_sites: 2,
          epsilon: 0.63,
          delta: 0.00002,
          ...
          guardrail_warnings: ["epsilon_warn_threshold_reached"],
          aggregated_participant_count: 2,
          available_participant_count: 3,
          sites: [
...
describe("FederationSection", () => {
  it("renders current and per-aggregation privacy budget summaries", () => {
    ...
    expect(screen.getAllByText("gaussian_rdp_poisson_subsampled").length).toBeGreaterThan(0);
    ...
    expect(screen.getAllByText("1.00e-5").length).toBeGreaterThan(0);
    expect(screen.getByText("Warning")).toBeInTheDocument();
    expect(screen.getByText(/warn ε 0.900 · max ε 1.200 · epsilon warn threshold reached/i)).toBeInTheDocument();
    expect(screen.getAllByText(/site-local training scope, client delta noise, gaussian rdp, poisson subsampling, no secure aggregation/i).length).toBeGreaterThan(0);
