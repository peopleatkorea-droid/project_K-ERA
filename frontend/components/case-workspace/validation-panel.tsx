...
function ValidationPanelInner({
  ...
  const consensusSnapshotContent = useMemo(() => {
    if (successfulComparisons.length <= 1) {
      return null;
    }
    return (
  ...
  const comparisonCardsContent = useMemo(
    () =>
    ...
    [common.notAvailable, formatProbability, locale, modelCompareResult?.comparisons],
  );
  const stepOneModelSnapshotContent = useMemo(() => {
    if (comparedModelsSnapshot.length === 0) {
      return null;
    }
    return (
      <Card
    ...
    anchorModelVersionId,
    common.notAvailable,
    comparedModelsSnapshot,
    formatProbability,
    locale,
  ]);
  const stepOnePrimarySummaryContent = useMemo(() => {
    if (!validationResult) {
      return null;
    }
    return (
      <div className="grid gap-4">
        ...
        <p className="m-0 text-sm leading-6 text-muted">
          ...
          {validationResult.model_version.version_name} ({validationModelLabel}){" "}
  ...
  const postMortemContent = useMemo(() => {
    if (!postMortem) {
      return null;
    }
    return (
      ...
      >
        ...
        {predictionSnapshot ? (
          ...
          >
            ...
            <MetricGrid columns={4}>
              ...
              />
              <MetricItem
                value={predictionSnapshot.crop_mode ?? common.notAvailable}
            ...
            <MetricGrid columns={3}>
              <MetricItem
                value={
                  predictionSnapshot.classifier_embedding?.embedding_id ??
                  common.notAvailable
                }
                label={pick(
                  locale,
        ...
        {structuredAnalysis && structuredScores ? (
          ...
          >
            ...
            <MetricGrid columns={4}>
              ...
              <MetricItem
                value={String(
                  structuredScores.similar_case_count ?? common.notAvailable,
                )}
                label={pick(locale, "similar cases", "유사 케이스")}
              />
              <MetricItem
                value={String(
                  structuredScores.text_evidence_count ?? common.notAvailable,
                )}
            ...
            {peerConsensus ? (
              <Card as="div" variant="panel" className="grid gap-3 p-4">
                ...
                <MetricGrid columns={4}>
                  <MetricItem
                    value={String(
                      peerConsensus.models_evaluated ?? common.notAvailable,
                    )}
                    label={pick(locale, "models", "모델 수")}
                  />
                  <MetricItem
                    value={peerConsensus.leading_label ?? common.notAvailable}
                    label={pick(locale, "leading label", "우세 라벨")}
                  />
                  <MetricItem
                    value={formatRatio(
                  ...
                  <MetricItem
                    ...
                    label={pick(locale, "vote entropy", "의견 엔트로피")}
                  />
                </MetricGrid>
              </Card>
            ) : null}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          >
            <SectionHeader
              title={pick(locale, "Contradictory evidence", "상충 신호")}
              titleAs="h4"
              className={docSectionHeadClass}
            />
            <div className="grid gap-3">
  ...
  return (
    <>
      ...
      {view !== "judgment" ? (
        ...
        {modelCompareResult ? (
          <div className="grid gap-4">
            {consensusSnapshotContent}
            {comparisonCardsContent}
          </div>
        ) : null}
        </Card>
      ) : null}
