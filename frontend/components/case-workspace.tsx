...
import {
  buildCaseWorkspaceHeaderAlertsProps,
  buildCaseWorkspaceHeaderFrameProps,
} from "./case-workspace/case-workspace-header-props";
...
export function CaseWorkspace({
  ...
  const visibleSites = useMemo(() => filterVisibleSites(sites), [sites]);
  ...
  const selectedSiteLabel = useMemo(
    () =>
      selectedSiteId
        ...
        : null,
    [selectedSiteId, selectedSiteRecord],
  );
  ...
  const toggleLiveLesionMask = useCallback(() => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && "requestIdleCallback" in window) {
      const idleHandle = window.requestIdleCallback(mountAnalysisSection, {
        timeout: 280,
      });
      return () => {
        cancelled = true;
    ...
    const timeoutId = window.setTimeout(mountAnalysisSection, 120);
    return () => {
      cancelled = true;
  ...
  const idlePatientListPrefetchCases = useMemo(
    () => patientListRows.slice(0, 2).map((row) => row.latest_case),
    [patientListRows],
  );
  ...
  const patientListViewProps = useMemo(
    () =>
      buildPatientListViewProps({
        locale,
        localeTag,
    ...
    [
      ...
      handleOpenImageTextSearchResult,
      handleOpenMedsamArtifactBacklog,
      handlePatientListPageChange,
      handlePatientListSearchChange,
      handlePatientScopeChange,
      handleRefreshMedsamArtifactStatus,
      handleDisableMedsamArtifactPanel,
      locale,
      localeTag,
  ...
  const headerFrameProps = useMemo(
    () =>
      buildCaseWorkspaceHeaderFrameProps({
        locale,
  ...
  const headerAlertsProps = useMemo(
    () =>
      buildCaseWorkspaceHeaderAlertsProps({
        localeTag,
        alertsPanelRef,
  ...
  const savedCaseViewProps = useMemo(
    () =>
      selectedCase
        ? buildSavedCaseViewProps({
            locale,
            ...
            representativeBusyImageId,
            formatSemanticScore,
            onToggleLiveLesionMask: toggleLiveLesionMask,
    ...
    [
      ...
      startFollowUpDraftFromSelectedCase,
      toggleFavoriteCase,
      toggleLiveLesionMask,
      translateOption,
    ],
  );
  const draftViewProps = useMemo(
    () =>
      buildDraftViewProps({
        locale,
        draft,
  ...
  const analysisSectionProps = useMemo(
    () =>
      buildCaseWorkspaceAnalysisSectionProps({
        locale,
        token,
  ...
  const contributionSectionProps = useMemo(
    () =>
      buildCaseWorkspaceContributionSectionProps({
        locale,
        mounted: analysisSectionMounted,
  ...
  const analysisSectionContent = useMemo(
    () => buildCaseWorkspaceAnalysisSectionContent(analysisSectionProps),
    [analysisSectionProps],
  );
  const selectedCasePanelContent = useMemo(
  ...
  const surfaceProps = useMemo(
    () =>
      buildCaseWorkspaceSurfaceProps({
        theme,
        toast,
        savedLabel: common.saved,
        actionNeededLabel: common.actionNeeded,
