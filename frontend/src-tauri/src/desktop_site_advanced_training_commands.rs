...
#[tauri::command]
pub(super) fn run_image_level_federated_round(
    payload: ImageLevelFederatedRoundCommandRequest,
) -> Result<JsonValue, String> {
    let site_id = payload.site_id.trim().to_string();
    if site_id.is_empty() {
        return Err("site_id is required.".to_string());
    ...
    let request_payload = json!({
        ...
        "model_version_id": payload.model_version_id.map(|value| value.trim().to_string()).filter(|value| !value.is_empty()),
        "epochs": payload.epochs.unwrap_or(1),
        "learning_rate": payload.learning_rate.unwrap_or(5e-5),
        "batch_size": payload.batch_size.unwrap_or(8),
    });
    if ml_sidecar_should_be_used() {
        return request_ml_sidecar_json("run_image_level_federated_round", request_payload);
    }
    request_local_api_json(
        HttpMethod::POST,
        &format!("/api/sites/{site_id}/training/federated/image-level"),
        request_payload
            .get("token")
            .and_then(|value| value.as_str())
            .unwrap_or(""),
...
#[tauri::command]
pub(super) fn fetch_image_level_federated_round_status(
    payload: ImageLevelFederatedRoundStatusCommandRequest,
) -> Result<JsonValue, String> {
    let site_id = payload.site_id.trim().to_string();
    if site_id.is_empty() {
        return Err("site_id is required.".to_string());
    ...
    if ml_sidecar_should_be_used() {
        return request_ml_sidecar_json("fetch_image_level_federated_round_status", request_payload);
    }
    ...
    request_local_api_json(
        HttpMethod::GET,
        &format!("/api/sites/{site_id}/training/federated/image-level/status"),
        request_payload
            .get("token")
            .and_then(|value| value.as_str())
            .unwrap_or(""),
...
#[tauri::command]
pub(super) fn run_visit_level_federated_round(
    payload: VisitLevelFederatedRoundCommandRequest,
) -> Result<JsonValue, String> {
    let site_id = payload.site_id.trim().to_string();
    if site_id.is_empty() {
        return Err("site_id is required.".to_string());
    ...
    if ml_sidecar_should_be_used() {
        return request_ml_sidecar_json("run_visit_level_federated_round", request_payload);
    }
    request_local_api_json(
        HttpMethod::POST,
        &format!("/api/sites/{site_id}/training/federated/visit-level"),
        request_payload
            .get("token")
            .and_then(|value| value.as_str())
            .unwrap_or(""),
...
#[tauri::command]
pub(super) fn fetch_visit_level_federated_round_status(
    payload: VisitLevelFederatedRoundStatusCommandRequest,
) -> Result<JsonValue, String> {
    let site_id = payload.site_id.trim().to_string();
    if site_id.is_empty() {
        return Err("site_id is required.".to_string());
    ...
    if ml_sidecar_should_be_used() {
        return request_ml_sidecar_json("fetch_visit_level_federated_round_status", request_payload);
    }
    let mut query = Vec::new();
    if let Some(model_version_id) = request_payload
        .get("model_version_id")
    ...
    request_local_api_json(
        HttpMethod::GET,
        &format!("/api/sites/{site_id}/training/federated/visit-level/status"),
        request_payload
            .get("token")
            .and_then(|value| value.as_str())
            .unwrap_or(""),
...
#[tauri::command]
pub(super) fn fetch_federated_retrieval_corpus_status(
    payload: FederatedRetrievalStatusCommandRequest,
) -> Result<JsonValue, String> {
    let site_id = payload.site_id.trim().to_string();
    if site_id.is_empty() {
        return Err("site_id is required.".to_string());
    ...
    if ml_sidecar_should_be_used() {
        return request_ml_sidecar_json("fetch_federated_retrieval_corpus_status", request_payload);
    }
    let mut query = Vec::new();
    if let Some(retrieval_profile) = request_payload
        .get("retrieval_profile")
        .and_then(|value| value.as_str())
    {
        query.push(("retrieval_profile", retrieval_profile.to_string()));
    }
    request_local_api_json(
        HttpMethod::GET,
        &format!("/api/sites/{site_id}/ai-clinic/retrieval-corpus/status"),
        request_payload
            .get("token")
            .and_then(|value| value.as_str())
            .unwrap_or(""),
...
#[tauri::command]
pub(super) fn sync_federated_retrieval_corpus(
    payload: FederatedRetrievalSyncCommandRequest,
) -> Result<JsonValue, String> {
    let site_id = payload.site_id.trim().to_string();
    if site_id.is_empty() {
        return Err("site_id is required.".to_string());
    ...
    let request_payload = json!({
        ...
        "token": payload.token,
        "execution_mode": payload.execution_mode.unwrap_or_else(|| "auto".to_string()),
        "retrieval_profile": payload.retrieval_profile.unwrap_or_else(|| "dinov2_lesion_crop".to_string()),
        "force_refresh": payload.force_refresh.unwrap_or(false),
    });
    if ml_sidecar_should_be_used() {
        return request_ml_sidecar_json("sync_federated_retrieval_corpus", request_payload);
    }
    request_local_api_json(
        HttpMethod::POST,
        &format!("/api/sites/{site_id}/ai-clinic/retrieval-corpus/sync"),
        request_payload
            .get("token")
            .and_then(|value| value.as_str())
            .unwrap_or(""),
