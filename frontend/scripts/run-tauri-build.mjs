...
function stopRunningReleaseArtifacts() {
  ...
  const script = [
    ...
    "    if ($_.Name -eq 'makensis.exe' -and $commandLine.Contains($nsisNeedle)) { return $true }",
    "    if ($_.Name -eq 'node.exe' -and $commandLine.Contains('@tauri-apps\\\\cli\\\\tauri.js') -and $commandLine.Contains(' build')) { return $true }",
    "    return $false",
    "  } |",
