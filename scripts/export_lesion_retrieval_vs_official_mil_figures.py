...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "artifacts" / "paper_figures" / "lesion_retrieval_vs_official_mil_20260331" / "visit_level"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export focused lesion-retrieval vs official-MIL visit-level figures."
    )
...
def build_model_performance_figure(results: list[dict[str, Any]], output_dir: Path) -> tuple[Path, Path]:
    ...
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title("Lesion Retrieval vs Official DINOv2 MIL", fontweight="bold")
    style_axes(ax)
...
def build_generalization_gap_figure(results: list[dict[str, Any]], output_dir: Path) -> tuple[Path, Path]:
    ...
    gap_specs = [
        ("Validation-Test AUROC Gap", auroc_gap, auroc_err, "AUROC gap"),
        ("Validation-Test Balanced Acc Gap", bal_gap, bal_err, "Balanced Acc gap"),
    ]
    for ax, (title, gaps, errs, ylabel) in zip(axes, gap_specs, strict=False):
        ...
        style_axes(ax)
    fig.suptitle("Generalization Gap: Lesion Retrieval vs Official DINOv2 MIL", fontweight="bold", y=1.02)
...
def build_roc_figure(results: list[dict[str, Any]], output_dir: Path) -> tuple[Path, Path]:
    ...
    for item in sorted(results, key=lambda row: row["test_auroc_mean"] or 0.0, reverse=True):
    ...
    ax.set_title("Out-of-Fold ROC: Lesion Retrieval vs Official DINOv2 MIL", fontweight="bold")
...
<head>
  <meta charset="utf-8" />
  <title>Lesion Retrieval vs Official MIL</title>
  <style>
...
<body>
  <h1>Lesion Retrieval vs Official DINOv2 MIL</h1>
...
def main() -> int:
    ...
    if len(results) != 2:
        raise RuntimeError("Expected exactly two result groups for lesion-retrieval vs MIL export.")
