...
DEFAULT_MAIN_CV_ROOT = REPO_ROOT / "artifacts" / "current_model_suite_cv_20260330_p73_5fold"
DEFAULT_ABLATION_ROOT = REPO_ROOT / "artifacts" / "official_retrieval_crop_ablation_cv_20260331"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "artifacts" / "paper_figures" / "official_retrieval_crop_ablation_cv_20260331"
...
METHOD_SPECS = [
    ...
    },
    {
        ...
        "label": "Official Retrieval (Cornea ROI)",
        "family": "Retrieval",
        "color": "#0f766e",
        "linestyle": "--",
        "source": "main",
    },
    {
        ...
        "label": "Official Retrieval (Full Frame)",
        "family": "Retrieval",
        "color": "#2563eb",
        "linestyle": "-.",
        "source": "ablation",
    },
    {
        ...
        "label": "Official Retrieval (Lesion Crop)",
        "family": "Retrieval",
        "color": "#dc2626",
        "linestyle": "-",
        "source": "ablation",
    },
    {
        ...
        "label": "Official Retrieval (Cornea+Lesion Fusion)",
        "family": "Retrieval",
        "color": "#7c3aed",
        "linestyle": ":",
        "source": "ablation",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export visit-level retrieval ablation figures and summary tables."
    )
    parser.add_argument("--main-cv-root", type=Path, default=DEFAULT_MAIN_CV_ROOT)
    parser.add_argument("--ablation-root", type=Path, default=DEFAULT_ABLATION_ROOT)
    parser.add_argument("--out-dir", type=Path, default=DEFAULT_OUTPUT_ROOT / "visit_level")
...
def collect_fold_rows(root: Path, spec: dict[str, str]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for path in sorted(root.glob(f"fold_*/{spec['name']}/result.json")):
        ...
        rows.append(
            {
                ...
                "evaluation_unit": str(result.get("evaluation_unit") or "visit"),
...
def summarize_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for spec in METHOD_SPECS:
        ...
        sens_mean, sens_std = mean_std([item["test_metrics"].get("sensitivity") for item in items])
        spec_mean, spec_std = mean_std([item["test_metrics"].get("specificity") for item in items])
        gap_auroc = [
            (
        ...
        gap_bal = [
            (
                ...
                if item["val_metrics"].get("balanced_accuracy") is not None
                and item["test_metrics"].get("balanced_accuracy") is not None
                else None
            )
            for item in items
        ]
        ...
        summaries.append(
            {
                ...
                "test_sensitivity_mean": sens_mean,
                "test_sensitivity_std": sens_std,
                "test_specificity_mean": spec_mean,
                "test_specificity_std": spec_std,
...
def build_model_performance_figure(results: list[dict[str, Any]], output_dir: Path) -> tuple[Path, Path]:
    ...
    fig, ax = plt.subplots(figsize=(max(10.0, len(ordered) * 1.45), 6.0))
    bars_auroc = ax.bar(
        x - width / 2,
        test_auroc,
        width,
    ...
    ax.set_ylim(0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title("Visit-Level 5-Fold CV Performance", fontweight="bold")
    style_axes(ax)
...
def build_generalization_gap_figure(results: list[dict[str, Any]], output_dir: Path) -> tuple[Path, Path]:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(max(13.0, len(ordered) * 1.55), 5.9))
    gap_specs = [
    ...
    for ax, (title, gaps, errs, ylabel) in zip(axes, gap_specs, strict=False):
        ax.bar(x, gaps, color=colors, edgecolor="#0f172a", linewidth=0.6, yerr=errs, capsize=4)
        ...
        style_axes(ax)
    fig.suptitle("Visit-Level 5-Fold CV Generalization Gap", fontweight="bold", y=1.02)
...
def build_roc_figure(results: list[dict[str, Any]], output_dir: Path) -> tuple[Path, Path]:
    ...
    ax.set_title("Visit-Level Out-of-Fold ROC Curves", fontweight="bold")
...
def write_html_report(
    ...
    *,
    ...
    ordered = sorted(results, key=lambda item: item["test_auroc_mean"] or 0.0, reverse=True)
    rows = "\n".join(
        f"<tr><td>{item['label']}</td><td>{item['family']}</td><td>{item['completed_folds']}</td><td>{(item['test_auroc_mean'] or 0.0):.3f} ± {(item['test_auroc_std'] or 0.0):.3f}</td><td>{(item['test_bal_acc_mean'] or 0.0):.3f} ± {(item['test_bal_acc_std'] or 0.0):.3f}</td><td>{(item['test_sensitivity_mean'] or 0.0):.3f}</td><td>{(item['test_specificity_mean'] or 0.0):.3f}</td><td>{(item['gap_auroc_mean'] or 0.0):+.3f}</td><td>{(item['gap_bal_acc_mean'] or 0.0):+.3f}</td></tr>"
        for item in ordered
    )
    figure_sections = "\n".join(
        f"""
...
<head>
  <meta charset="utf-8" />
  <title>Official Retrieval Crop Ablation Figures</title>
  <style>
...
<body>
  <h1>Official Retrieval Crop Ablation 5-Fold CV Figures</h1>
  <p class="meta">Visit-level comparison on {dataset_summary.get('n_patients')} patients / {dataset_summary.get('n_visits')} visits / {dataset_summary.get('n_images')} images.</p>
  <div class="figures">
    {figure_sections}
  </div>
  <h2>Summary Table</h2>
...
def main() -> int:
    args = parse_args()
    main_cv_root = args.main_cv_root.expanduser().resolve()
    ablation_root = args.ablation_root.expanduser().resolve()
    output_dir = args.out_dir.expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)

    dataset_summary = load_json(main_cv_root / "queue_status.json").get("dataset_summary") or {}
    rows: list[dict[str, Any]] = []
    for spec in METHOD_SPECS:
        root = main_cv_root if spec["source"] == "main" else ablation_root
        rows.extend(collect_fold_rows(root, spec))
    results = summarize_rows(rows)
    if not results:
        raise RuntimeError("No visit-level result.json files were found for the requested comparison.")
