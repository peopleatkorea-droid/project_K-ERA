...
class ThreeScaleLesionGuidedFusionKeratitis(nn.Module):
    def __init__(
        self,
        architecture: str,
        *,
        ...
        super().__init__()
        if not is_three_scale_lesion_guided_fusion_architecture(architecture):
            raise ValueError(f"Three-scale lesion-guided fusion architecture is invalid: {architecture}")
        backbone_name = lesion_guided_fusion_backbone(architecture)
        if not backbone_name:
            raise ValueError(f"Unable to resolve three-scale lesion-guided fusion backbone from architecture: {architecture}")
        self.architecture = architecture
        self.backbone_name = backbone_name
        self.backbone_adapter = LesionGuidedFusionBackboneAdapter(backbone_name, init_mode=init_mode)
        self.backbone = self.backbone_adapter.encoder
    ...
    def forward_features(
        self,
        cornea_inputs: torch.Tensor,
        medium_inputs: torch.Tensor,
        lesion_inputs: torch.Tensor,
        lesion_masks: torch.Tensor | None = None,
    ) -> torch.Tensor:
        cornea_map, cornea_global = self._encode_map_and_global(cornea_inputs, branch_name="cornea")
        medium_global = self._encode_global(medium_inputs, branch_name="medium")
        lesion_global = self._encode_global(lesion_inputs, branch_name="lesion")
        medium_feature = self.medium_projection(medium_global)
        lesion_feature = self.lesion_projection(lesion_global)
        context_feature = self.context_projection(torch.cat([medium_feature, lesion_feature], dim=1))
        channel_gate = self.channel_gate(context_feature).unsqueeze(-1).unsqueeze(-1)
        soft_mask = self._soft_mask(
            lesion_masks,
            cornea_map.shape[-2:],
            batch_size=cornea_map.shape[0],
        ...
        fused_feature = torch.cat(
            [cornea_global, guided_global, context_feature, medium_feature, lesion_feature],
            dim=1,
        )
        return self.fusion_projection(fused_feature)
    ...
    def forward(
        ...
        return self.classifier(self.forward_features(cornea_inputs, medium_inputs, lesion_inputs, lesion_masks))
