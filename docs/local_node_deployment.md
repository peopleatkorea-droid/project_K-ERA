...
- `uv`가 관리하는 repo-root `.venv` 기반 Python 실행 환경
...
.\scripts\configure_local_node.ps1 `
  -ControlPlaneApiBaseUrl "https://<control-plane-host>/control-plane/api" `
...
- 모든 병원 노드는 같은 `ControlPlaneApiBaseUrl`을 사용해야 합니다.
...
- `KERA_CONTROL_PLANE_API_BASE_URL`: 중앙 Next.js control-plane API 권장
- `KERA_LOCAL_CONTROL_PLANE_DATABASE_URL`: 병원 노드에서 중앙 metadata projection/cache를 저장하는 로컬 SQLite 권장
...
기본 스크립트는 `pyproject.toml` + `uv.lock`을 source of truth로 사용하고, 아래 extras/profile을 해석합니다.

- `cpu`: CPU용 torch/torchvision
- `gpu`: CUDA 12.8용 torch/torchvision
- `dev`: 개발/테스트 보조 패키지
...
이 스크립트는 `uv venv .venv --python 3.11` + `uv sync --frozen ...` 흐름을 감싸는 래퍼입니다.
