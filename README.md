...
- **웹 포털 다운로드**: 계속 OneDrive + `데스크톱 설치본 관리` 등록 흐름을 사용합니다.
- **설치된 데스크톱 앱 자동업데이트**: `v1.0.0`부터는 앱 시작 시 GitHub Release updater를 확인하는 혼합 운영을 기준으로 합니다.
- 즉, **새 설치는 웹 포털/OneDrive**, **이미 설치된 앱의 후속 업데이트는 GitHub Release**로 이해하면 됩니다.
- 자동업데이트를 쓰려면 `frontend/src-tauri/tauri.conf.json`의 updater public key와 GitHub Actions secret `TAURI_SIGNING_PRIVATE_KEY`가 같은 키쌍이어야 합니다.
...
- 웹 포털의 새 설치본 배포는 계속 `데스크톱 설치본 관리` + OneDrive 등록 흐름을 기준으로 합니다.
- 동시에 설치된 데스크톱 앱의 후속 업데이트는 GitHub Release updater를 사용하는 혼합 운영을 지원합니다.
- 따라서 `desktop-release` workflow는 더 이상 참고용이 아니라, 설치된 앱 자동업데이트 경로의 실운영 구성요소입니다.
...
- 승인된 site access가 있는 사용자는 데스크톱 실행 경로에서 `researcher` 권한으로 AI 실행이 가능하도록 role 해석을 정규화합니다.
...
현재 데스크톱 저장 케이스 분석은 아래 **3단계 허브**를 기준으로 동작합니다.

1. **단일 케이스 판정**
   - 기준 모델(anchor)로 예측 라벨, 신뢰도, 저장 가능한 검토 이미지(crop / Grad-CAM)를 만듭니다.
   - 함께 선택된 비교 모델 결과도 같은 화면에서 바로 확인합니다.
2. **모델 간 합의 확인**
   - 여러 준비 완료 모델이 같은 결론인지 비교합니다.
   - 기준 모델과 비교 모델 집합을 분리해, 이미지 생성용 모델과 주 판정용 모델을 다르게 둘 수 있습니다.
3. **유사 환자 검토**
   - similar-patient retrieval을 먼저 실행합니다.
   - 필요할 때만 텍스트 근거 / 가이드를 확장해서 불러오고, 3D 클러스터 맵으로 현재 케이스 위치를 확인합니다.
...
- **3D cluster map**: 현재 케이스와 유사 환자를 임베딩 클러스터 위치로 회전 탐색
...
- 현재 권장 운영은 `웹 포털은 OneDrive 설치본 안내`, `설치된 앱 자동업데이트는 GitHub Release 서명 업데이트`의 혼합 방식입니다.
