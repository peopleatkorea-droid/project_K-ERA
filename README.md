...
- basic composition 기반 DP accountant와 누적 budget snapshot은 있습니다. 다만 RDP/PRV 같은 고급 accountant는 아직 없습니다.
...
- basic composition 기반 DP accountant와 aggregation별 누적 privacy budget 보고는 있습니다. 다만 고급 accountant와 secure aggregation은 아직 없습니다.
...
| `KERA_FEDERATED_DELTA_NOISE_MULTIPLIER` | clipping 후 추가할 Gaussian noise 배수. `KERA_FEDERATED_DP_ACCOUNTANT_DELTA`와 같이 쓰면 basic composition 기반 DP accountant와 누적 budget snapshot을 계산 |
...
| `KERA_REQUIRE_FORMAL_DP_ACCOUNTING` | `true`면 DP accountant가 없는 빌드에서 FL round/aggregation을 시작하지 않음. 현재는 basic composition accountant까지만 구현되어 있음 |
